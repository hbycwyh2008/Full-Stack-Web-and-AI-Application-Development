...
Use only these six items from the workbook Evidence Bank:

| Item | Evidence |
|---|---|
| **A** | Compare two search algorithms as input grows. |
| **E** | Design routing that still works after a network failure. |
| **I** | Observe users and redesign an interface to reduce errors. |
| **K** | Organize a software project so many developers can change it safely. |
...
3. identify which items show that CS is broader than programming;
4. choose one disputed placement and justify both positions.

Teacher asks evidence questions:

- “Which words in the item support that field?”
...
- **G:** Analyze an unauthorized-access path and design protections.
- **L:** Use tests and code review to decide whether a change is safe to merge.
- **Q:** Investigate whether an automated decision treats groups unfairly.
...
If GitHub is already available, suggested commit:

```text
Complete Lesson 00 computer science discovery
```

Otherwise save locally for later upload. There is no separate paper submission.
